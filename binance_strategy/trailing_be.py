...
class TrailingBE(ABinanceStrategy):
    ...
    def logic(self):
        ...
        if cash != 0 and pv == 0:
            ...
            if returns > -self.deadpoint and returns < -self.stoploss and returns > -self.deadpoint and new_orders.index.size < 1:
                self.umf.cancel_open_orders(self.ticker)
                sleep(10)
                ...
                if float(starting_amount) > 0:
                    self.umf.change_leverage(self.ticker,self.leverage)
                    self.umf.new_order(**bpc.long_limit_close(self.ticker,starting_amount,breakeven_price*(1-.001)))
                else:
                    self.umf.change_leverage(self.ticker,self.leverage)
                    self.umf.new_order(**bpc.short_limit_close(self.ticker,starting_amount,breakeven_price*(1+.001)))